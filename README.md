...
### 🖱️ Web Upload Tool (New!) / Webアップロードツール（新機能！）
```bash
# Start upload server / アップロードサーバーを起動
...
**Features / 機能:**
- **Drag & Drop Interface**: Drag images directly to upload
  - **ドラッグ&ドロップインターフェース**: 画像を直接ドラッグしてアップロード
...
- **Real-time Progress**: See upload progress for each file
  - **リアルタイム進行状況**: 各ファイルのアップロード進行状況を表示
- **Auto Git Commit**: Automatically commits and pushes to GitHub
  - **自動Gitコミット**: 自動でGitHubにコミット・プッシュ
- **Cyberpunk UI**: Matches gallery design aesthetic
  - **サイバーパンクUI**: ギャラリーデザインに統一されたインターフェース
...
│   ├── upload-server.js   # Web upload server / Webアップロードサーバー
│   └── setup.js           # Initial setup / 初期セットアップ
├── upload-tool.html        # Drag & drop upload interface / ドラッグ&ドロップアップロードインターフェース
├── config/
